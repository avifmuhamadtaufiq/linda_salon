...
def dashboard(request):
    ...
    trx_selesai_hari_ini = Transaksi.objects.filter(
        status="selesai", tanggal_kembali_aktual=today
    ).count()
    pengembalian_hari_ini = Transaksi.objects.filter(
        status="disewa", tanggal_kembali=today
    ).count()
...
def barang_detail(request, pk):
    ...
    riwayat = barang.detail_transaksi.select_related("transaksi").order_by(
        "-transaksi__created_at"
    )[:10]
    return render(
        request, "inventory/barang_detail.html", {"barang": barang, "riwayat": riwayat}
    )
...
def barang_create(request):
    ...
    return render(
        request, "inventory/barang_form.html", {"form": form, "title": "Tambah Barang"}
    )
...
def barang_edit(request, pk):
    ...
    return render(
        request,
        "inventory/barang_form.html",
        {"form": form, "title": "Edit Barang", "barang": barang},
    )
...
def gudang_list(request):
    ...
    return render(request, "inventory/gudang_list.html", {"gudang_list": gudang})
...
def gudang_create(request):
    if request.method == "POST":
        ...
        if nama:
            Gudang.objects.create(nama=nama, alamat=alamat, keterangan=keterangan)
    ...
    return render(request, "inventory/gudang_form.html", {"title": "Tambah Gudang"})
...
def gudang_edit(request, pk):
    ...
    return render(
        request,
        "inventory/gudang_form.html",
        {"title": "Edit Gudang", "gudang": gudang},
    )
...
def gudang_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("gudang_list")
    return render(request, "inventory/gudang_confirm_delete.html", {"gudang": gudang})
