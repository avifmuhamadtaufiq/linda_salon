...
class KategoriModelTest(TestCase):
    def setUp(self):
        ...
        self.kategori = Kategori.objects.create(
            nama="Kursi", deskripsi="Kursi pernikahan"
        )
...
class BarangFormTest(TestCase):
    def setUp(self):
        ...
        self.gudang = Gudang.objects.create(
            nama="Gudang Andir", alamat="Jl. Andir No. 1"
        )
...
class DashboardViewTest(TestCase):
    def setUp(self):
        ...
        # Buat user admin
        self.user = User.objects.create_user(username="admin_test", password="admin123")
...
class BarangViewTest(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="admin_test2", password="admin123"
        )
        UserProfile.objects.create(user=self.user, role="admin")
    ...
    def test_barang_detail_tampil(self):
        ...
        response = self.client.get(reverse("barang_detail", args=[self.barang.pk]))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Kursi Tiffany")

    def test_barang_delete(self):
        # Test hapus barang
        response = self.client.post(reverse("barang_delete", args=[self.barang.pk]))
        self.assertEqual(response.status_code, 302)
...
class GudangViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(
            username="admin_gudang", password="admin123"
        )
        UserProfile.objects.create(user=self.user, role="admin")
    ...
    def test_gudang_delete(self):
        ...
        response = self.client.post(reverse("gudang_delete", args=[self.gudang.pk]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_filter_barang_by_gudang(self):
        ...
        response = self.client.get(reverse("barang_list"), {"gudang": self.gudang.pk})
        self.assertContains(response, "Kursi Andir")
        self.assertNotContains(response, "Kursi Kubang")

        # Filter by gudang kubang
        response = self.client.get(reverse("barang_list"), {"gudang": gudang2.pk})
