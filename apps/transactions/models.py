...
class Transaksi(models.Model):
    ...
    diskon = models.DecimalField(
        max_digits=15, decimal_places=0, default=0, verbose_name="Diskon"
    )
    ...
    dibuat_oleh = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="transaksi"
    )
    ...
    def total_sudah_bayar(self):
        ...
        cicilan = self.pembayaran.aggregate(total=Sum("jumlah"))["total"] or 0
...
class Pembayaran(models.Model):
    ...
    dicatat_oleh = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="pembayaran_dicatat"
    )
    created_at = models.DateTimeField(auto_now_add=True)
