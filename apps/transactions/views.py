...
def hitung_jumlah_hari(tanggal_sewa, tanggal_kembali):
    try:
        ...
        tgl_kembali = datetime.datetime.strptime(
            str(tanggal_kembali), "%Y-%m-%d"
        ).date()
...
def transaksi_list(request):
    ...
    pengguna_list = (
        User.objects.filter(transaksi__isnull=False).distinct().order_by("first_name")
    )
    ...
    return render(
        request,
        "transactions/transaksi_list.html",
        {
...
def transaksi_detail(request, pk):
    ...
    return render(
        request,
        "transactions/transaksi_detail.html",
        {
            "transaksi": transaksi,
            "detail": detail,
        },
...
def transaksi_create(request):
            ...
            with transaction.atomic():
                ...
                if pelanggan_id:
                    ...
                    if nama and hp:
                        pelanggan_obj, created = Pelanggan.objects.get_or_create(
                            hp=hp, defaults={"nama": nama, "alamat": alamat}
                        )
                        if not created:
                ...
                for b_id, jml in zip(barang_ids, jumlah_list):
                    ...
                    if jml > barang.stok_tersedia:
                        raise ValueError(
                            f"Stok {barang.nama} tidak mencukupi (tersedia: {barang.stok_tersedia})"
                        )

                    harga_satuan = Decimal(str(barang.harga_sewa))
                    subtotal = (
                        harga_satuan * Decimal(str(jml)) * Decimal(str(jumlah_hari))
                    )
                    ...
                    DetailTransaksi.objects.create(
                        ...
                        subtotal=subtotal,
                        kondisi_keluar=request.POST.get(
                            f"kondisi_keluar_{b_id}", "Baik"
                        ),
                    )
                    total += subtotal
    ...
    return render(
        request,
        "transactions/transaksi_form.html",
        {
...
def transaksi_siap_diambil(request, pk):
    ...
    if not cek_akses_transaksi(request.user, transaksi):
        messages.error(
            request, "Anda tidak memiliki akses untuk memproses transaksi ini."
        )
    ...
    return render(
        request,
        "transactions/transaksi_konfirmasi.html",
        {
            "transaksi": transaksi,
            "aksi": "siap_diambil",
...
def transaksi_kembali(request, pk):
    ...
    return render(
        request,
        "transactions/transaksi_kembali.html",
        {
            "transaksi": transaksi,
            "detail": transaksi.detail.select_related("barang"),
        },
...
def transaksi_batal(request, pk):
    ...
    if not cek_akses_transaksi(request.user, transaksi):
        messages.error(
            request, "Anda tidak memiliki akses untuk membatalkan transaksi ini."
        )
        return redirect("transaksi_detail", pk=transaksi.pk)
    ...
    if request.method == "POST":
        ...
        if not alasan:
            ...
            return render(
                request,
                "transactions/transaksi_batal.html",
                {"transaksi": transaksi, "error": "Alasan pembatalan wajib diisi."},
            )
...
def jadwal_view(request):
    ...
    siap_diambil = Transaksi.objects.filter(status="siap_diambil").order_by(
        "tanggal_sewa"
    )
...
def transaksi_invoice(request, pk):
    ...
    return render(
        request,
        "transactions/transaksi_invoice.html",
        {
            "transaksi": transaksi,
            "detail": detail,
            "generated_at": timezone.now(),
...
def tambah_pembayaran(request, pk):
    ...
    if transaksi.status == "batal":
        messages.error(
            request, "Transaksi yang dibatalkan tidak bisa ditambah pembayaran."
        )
        return redirect("transaksi_detail", pk=transaksi.pk)
    ...
    if request.method == "POST":
        ...
        if jumlah <= 0:
            messages.error(request, "Jumlah pembayaran harus lebih dari 0.")
            return redirect("transaksi_detail", pk=transaksi.pk)
...
def hapus_pembayaran(request, pk, pembayaran_pk):
    """Hapus pembayaran"""
    transaksi = get_object_or_404(Transaksi, pk=pk)
    pembayaran = get_object_or_404(Pembayaran, pk=pembayaran_pk, transaksi=transaksi)

    # Hanya admin yang bisa hapus pembayaran
    if request.user.profile.role != "admin":
        messages.error(request, "Hanya admin yang bisa menghapus pembayaran.")
        return redirect("transaksi_detail", pk=transaksi.pk)
